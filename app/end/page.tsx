...
type LastGame = {
  ...
  stats?: {
    ...
    steps?: number;
    byKind?: { 'fire-goblin': number; 'water-goblin': number; 'water-goblin-spear': number; 'earth-goblin': number; 'earth-goblin-knives': number; ghost: number; 'stone-exciter': number };
  };
};
...
export default function EndPage() {
  ...
  if (last.stats?.byKind) {
    Object.entries(last.stats.byKind as Record<string, number>).forEach(([enemyType, count]) => {
      ...
      if (n > 0) {
        const emoji = ({ ghost: '👻', 'fire-goblin': '👹', 'water-goblin': '🧊', 'water-goblin-spear': '🔱', 'earth-goblin': '🟤', 'earth-goblin-knives': '🗡️', 'stone-exciter': '🗿' } as Record<string, string>)[enemyType] || '👹';
