...
- [ ] New enemy - snake - [x] assign a new enemy type to the snake (Health: 2, Damage: 1) - [x] assign a new condition type of poison. It will work like this: a snake will poison you if it attacks you. The poison will deal 1 damage per 8 steps/turns. - [ ] The snake will start inside a pot and will attack once you open the pot. - [x] Use the enemey snake assets. It should use the coiled asset when not moving and the uncoiled asset when moving. - [ ] A snake will be generated in 33% of the rooms. It will start inside a pot 50% of that time. - [ ] If generated outside a pot it doesn't move until you are about 5 tiles away. - [ ] Taking the potion will cure the poison. When a player is poisoned, they will have a green aura around them and the tile grid will have a poisoned effect and the health bar will display poisoned hearts assets. - [x] The snake will move on the floor once freed from the pot. It will not chase the player, but inside move away from it when possible. If the snake doesn't start in the pot it will move around randomly until it sees the player and moves away. - [ ] Being poisioned doesn't affect your final score or grade. Killing snakes doesn't affect your final score or grade. Snakes present as enemies doesn't raise the max score but does raise the max damage dealt and is included in the kill count. - [ ] The daily end game screen has a cause of death section, this need to show if the hero dies from poison. Show the snake moving asset in that case.
...
- [ ] A door that take you to a second room. It could contain a new enemy or a new challenge. I like a giant boss enemy to screw you over, but the chance of reward is high too so you are inclined to take the risk.
