...
/**
 * The floor as it stands once every colour-lock puzzle on it is solved (the gate spikes
 * retract). The daily colour puzzle deliberately seals the exit key behind a solvable gate —
 * its switches are all reachable and freely cyclable, so all-same is always achievable — which
 * a walk-only flood fill reads as a severance because it cannot press switches. Opening ONLY the
 * colour-lock gate tiles (never the switch-gate bed) keeps every check below a true test of the
 * switch gate: if a switch gate ever walled off an essential, its spikes are still up here and
 * the assertion still fails.
 */
function withColorGatesOpen(state: GameState): MapData {
  const gates = new Set<string>();
  for (const lock of state.colorLocks ?? [])
    for (const [gy, gx] of lock.gates) gates.add(`${gy},${gx}`);
  if (gates.size === 0) return state.mapData;
  return {
    ...state.mapData,
    subtypes: state.mapData.subtypes.map((row, y) =>
      row.map((cell, x) =>
        gates.has(`${y},${x}`) ? cell.filter((s) => s !== TileSubtype.SPIKES) : cell
      )
    ),
  };
}
...
describe("switch gate placement on generated daily floors", () => {
  ...
  it("never severs an essential tile from the hero", () => {
    for (const { seed, floor, built, plan } of cases) {
      ...
      const reach = safeReach(withColorGatesOpen(state), built.hero);
